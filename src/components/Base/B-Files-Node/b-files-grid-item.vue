<template>
	<ul
		...
		<img
			...
			:src="src"
		/>
<!--		:src="fetchFileIcon(file.fileType, file.fileName || file.folderName, file.tableKind)"-->
...
export default {
	...
	methods: {
		...
		fileDrop(e) {
			...
			// 判断所框选拖拽的文件中是否包含要放置的文件夹本身
			/*
			* 由于W3C本身规则限制，e.dataTransfer中set的数据在dragenter、dragmove、dragleave事件中均为protected状态，
			* 所以无法访问，只有在drop事件中该数据才为read状态
			* */
	...
	computed: {
		...
		src() {
			if(this.file.fileKind === '图片' || this.file.fileKind === '视频') {
				return this.getThumnUrlOfImg(this.file.id)
			}else {
