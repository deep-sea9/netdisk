<template>
    <div class="out-container" ref="out-container" style="user-select: none;height: 100%">
        <el-table :data="data" style="width: 100%"
            ...
            <el-table-column prop="fileName" label="文件名" show-overflow-tooltip sortable="custom">
                <div draggable="true"
                     class="table-column"
                     slot-scope="scope"
                     @mousedown.stop="() => {}"
...
<script>
    ...
    import {RIGHT_CONTEXT, RIGHT_CONTEXT_DB_SELECTION} from "@/utils/constants.js";
    ...
    export default {
        ...
        methods: {
            ...
            fileDragStart(e, file) {
                ...
                // e.dataTransfer.setData('DownloadURL', `http://tmmysql.netiler.com/netdisk/ndk_apis/files/download?fileId=${file.id}`)
            },
            fileDragOver(e, file) {
            ...
            drawSelectArea(e) {
                ...
                if(clientX === this.downX && clientY === this.downY) return // 为防止chrome浏览器mousedown时触发mousemove事件
            ...
            handleSelectionChange(selection) {
                ...
                if (selection.length > 1) {
                    this.fileLimits = RIGHT_CONTEXT_DB_SELECTION
                } else {
                    this.fileLimits = RIGHT_CONTEXT;
                }
            },
            ...
            rowClick(row) {
                if(this.shiftKeyDown) {
                    ...
                    const firstIndex = this.data.findIndex(f => f.id === this.firstSelection.id)
                    const lastIndex = this.data.findIndex(f => f.id === this.lastSelection.id)

                    let selections

                    if(firstIndex > lastIndex) {
                        selections = this.data.slice(lastIndex, firstIndex + 1)
                    }else {
                        selections = this.data.slice(firstIndex, lastIndex + 1)
                    }
