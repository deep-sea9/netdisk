<template>
    <div class="b-files-header">
        <div>
            ...
            <span v-for="(fold, index) in indexList" :key="fold.id" @click="toPrevious(fold, index)" :class="{
                'no-last': index < indexList.length - 1
            }">{{ fold.folderName }} <span v-if="index < indexList.length - 1">></span></span>
        </div>
        <div class="operate">
            <b-icon
                    v-if="fileType === 'all'"
    ...
    export default {
        ...
        methods: {
            ...
            toggleIsGrid(isGrid) {
                ...
                // console.log('routeName', this.$route.name, gridConfig);
                this.$store.commit('app/SET_PUBLIC_AND_GROUP_IS_GRID', gridConfig)
