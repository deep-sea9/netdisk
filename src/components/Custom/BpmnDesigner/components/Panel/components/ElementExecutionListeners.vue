    ...
    <el-collapse-item name="element-execution-listeners">
        <template #title>
            <collapse-title title="执行监听">
                <z-svg-icon icon-class="wireless"/>
            </collapse-title>
            <number-tag :value="listeners.length" margin-left="12px"/>
        </template>
        <div class="element-extension-listeners">
            <el-table size="small" border :data="listeners" style="width: 100%" height="200px">
            ...
            <el-button size="small" type="primary" class="inline-large-button" icon="el-icon-plus" @click="openListenerModel(-1)">
                添加执行监听
            </el-button>
        </div>

        <el-dialog :visible.sync="modelVisible" title="添加执行监听器" width="640px" append-to-body destroy-on-close>
            <el-form size="small" ref="formRef" :model="newListener" :rules="formRules" class="need-filled" aria-modal="true">
                ...
                <el-form-item path="type" label="监听器类型( Listener Type )">
                    <el-select v-model="newListener.type" @change="updateListenerType">
                        <el-option v-for="{ label, value } in listenerTypeOptions" :label="label" :value="value"
                                   :key="value"/>
                    </el-select>
                </el-form-item>
...
<script>
    import {listenerTypeOptions, scriptTypeOptions} from "@/store/bpmn/preset-configuration/enumsOption";
    import {
    ...
    } from "@/components/Custom/BpmnDesigner/bo-utils/executionListenersUtil";
    import {getScriptType} from "@/components/Custom/BpmnDesigner/bo-utils/scriptUtil";
    ...
    export default {
        ...
        components: { CollapseTitle, NumberTag },
        data() {
            return {
                modelVisible: false,
                listeners: [],
