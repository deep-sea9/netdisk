    ...
    <el-collapse-item name="element-extension-properties">
        <template #title>
            <collapse-title title="扩展属性">
                <z-svg-icon icon-class="extention"/>
            </collapse-title>
            <number-tag :value="extensions.length" margin-left="12px"/>
        </template>
        <div class="element-extension-properties">
            <el-table size="small" border :data="extensions" style="width: 100%" height="200px">
            ...
            <el-button size="small" type="primary" class="inline-large-button" icon="el-icon-plus" @click="openPropertyModel(-1)">
                添加扩展属性
            </el-button>
        </div>

        <el-dialog :visible.sync="modelVisible" title="添加扩展属性" width="640px" append-to-body destroy-on-close>
            <el-form size="small" ref="formRef" :model="newProperty" :rules="rules" aria-modal="true">
            ...
            <template #footer>
                ...
                <el-button size="small" type="primary" @click="addProperty">确 认</el-button>
            </template>
        </el-dialog>
    </el-collapse-item>
</template>
...
<script>
    ...
    } from "@/components/Custom/BpmnDesigner/bo-utils/extensionPropertiesUtil";
