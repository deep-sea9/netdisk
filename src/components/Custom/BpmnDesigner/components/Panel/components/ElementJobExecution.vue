    ...
    <el-collapse-item name="element-external-task">
        <template #title>
            <collapse-title title="执行作业">
                <z-svg-icon icon-class="working"/>
            </collapse-title>
        </template>
        <div class="element-external-task">
            <edit-item v-if="tpVisible" label="任务优先级" :label-width="100">
                <el-input size="small" v-model="taskPriority" maxlength="32" @change="setExternalTaskPriority"/>
            </edit-item>
            <edit-item v-if="rtVisible" label="重试周期" :label-width="100">
                <el-input size="small" v-model="retryTimeCycle" maxlength="32" @change="setRetryTimeCycle"/>
            </edit-item>
        </div>
    </el-collapse-item>
</template>
...
<script>
    ...
    import {
        ...
        taskPriorityVisible
    } from "@/components/Custom/BpmnDesigner/bo-utils/jobExecutionUtil";
