...
<!--			</el-input>-->
		...
		<ui-layout-split mode="vertical" :show-collapse="false">
			<ui-layout-split-aside width="20">
				<el-input style="margin: 10px;width: 80%;" :maxlength="10" size="small" v-model="filterDepartment" @change="handleFilterDepartment" placeholder="请输入关键字进行过滤" />
				<el-tree
						:props="defaultProps"
						style="height: 100%;"
						ref="departTree"
						...
						:load="loadDepartmentData"
						@node-click="handleCheckedChange"
				>
...
<script>
import { modifyDiskSize, getPersonalDiskListOfOrgan } from '@/api/admin/personal'
import { getFirstLayerOrg, getOrgById } from '@/api/common.js'
...
export default {
	...
	methods: {
		...
		loadDepartmentData(node, resolve) {
			if(node.level === 0) {
				getFirstLayerOrg().then(res => {
					this.organId = res.data.id
					this.fetchDisks()
					resolve([{ ...res.data }])
				})
			}else {
				getOrgById(node.key).then(res => {
					return resolve(res.data.map(org => ({...org, isLeaf: org.childCount === 0})))
				})
			}
		},
		handleCheckedChange(data) {
			...
			this.pageNo = 1
			this.fetchDisks()
			// getPersonalDiskListOfOrgan({ pageNo: 1, pageSize: this.pageSize, organId: keys[0] }).then(res => {
			// 	console.log(res);
			// })
		},
		...
		handleFilterDepartment(key) {
			console.log(key);
			this.$refs['departTree'].filter(key)
		},
		...
		fetchDisks() {
			this.loadingInstance = this.$loading({ lock: true, text: '正在加载，请稍后...', spinner: 'el-icon-loading', background: 'rgba(0, 0, 0, 0.5)' })
			getPersonalDiskListOfOrgan({pageNo: this.pageNo, pageSize: this.pageSize, organId: this.organId, organName: this.organName}).then(res => {
