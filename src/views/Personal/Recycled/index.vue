...
export default {
	...
	methods: {
		fetchFiles() {
			getRecycleList({pageNo: this.pageNo, pageSize: this.pageSize}).then(res => {
		...
		handleRestore(file) {
			let fileIds
			if(this.selection.length<1 && file) {
				fileIds = [file.id]
			}else {
				fileIds = this.selection.map(file => file.id)
			}

			restoreFile({fileIds}).then(() => {
				this.fetchFiles()
		...
		clearRecycle() {
			this.$confirm('确认清空回收站吗？此操作将删除所有回收站数据且不可撤回，是否继续？', '提示', 'error').then(res => {
				if(res) {
					...
					deleteRecycle({fileIds: null}).then(() => {
						this.fetchFiles()
