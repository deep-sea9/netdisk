import {getFormatFileSize, setRandomId} from "@/utils";
...
export default {
    methods: {
        ...
        handleLinkDownload(file) {
            ...
            a.href = `${previewURL}/files/download?fileId=${file.fileId || file.id}`
            if(file.fileType) {
        ...
        handleCommonDownload(selection, downloadingList) {
            let fileIds = selection.map(file => file.id).join(",")
            this.$message.success('已添加至下载队列')
            let id = setRandomId()
            let fileName = setRandomId()
            downloadingList.push({
